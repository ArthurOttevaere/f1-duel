...
**It is drawn as content, not as wallpaper.** Full contrast, a red start line,
a caption. A trace at six per cent behind the headline would have been the
aurora again with extra steps.
...
carried the aurora too: one red source from the top, no second blob.
...
| `web/components/CircuitTrace.tsx` | The hero's ornament and its one light source. |
