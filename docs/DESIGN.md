...
- **The foot always invites.** The cue is a real score from a real Grand Prix
  when there is one, and `How the duel works ↓` when there is not — a hero
  that ends in nothing on a phone is a hero people take for the whole site.
