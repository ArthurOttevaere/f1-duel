...
export default async function NextRaceWidget() {
  ...
  return (
    <Link
      ...
      className="rise-in pressable glass-chip flex max-w-[90vw] flex-col items-center gap-4 rounded-2xl px-6 py-4 text-center transition-colors hover:border-line-hi sm:flex-row sm:gap-6 sm:text-left"
    >
      <div>
        <p className="font-mono text-[0.6rem] tracking-[0.2em] text-race uppercase">
          Next race · Round {race.round}
        </p>
        <p className="mt-1 text-base font-semibold sm:text-lg">{race.name}</p>
        {place && <p className="text-sm text-ink-dim">{place}</p>}
      </div>

      {/* Hairline divider, horizontal when the card stacks on mobile. */}
      <span
        aria-hidden
        className="h-px w-12 bg-line sm:h-12 sm:w-px"
      />
